...
class DSOM(MAP):
    ...
    def learn_data(self, data, lrate=0, sigma=0, elasticity=0, coefs=None):
        ...
        # print(data)
        # print("shape(self.codebook)", self.codebook.shape)
        # print("self.codebook", self.codebook)
        # print("shape(D)", np.shape((self.codebook - data) ** 2))
        # print("len(coefs)", len(coefs))
        # print("coefs : ", coefs)
        # print("type(coefs) : ", type(coefs))
        # print("type(data) : ", type(data))
        
        # init  = (((self.codebook - data)/coefs) ** 2)
        # test1 = (((self.codebook - data)/coefs.reshape(1, len(data))) ** 2)
        # test2 = (((self.codebook - data)/coefs.reshape(1, 1, len(data))) ** 2)
        # print("init = coefs.reshape(1, len(data) ?  ", np.array_equal(init,test1))
        # print("init = coefs.reshape(1, 1, len(data) ?  ", np.array_equal(init,test2))
        ...
        # Z = np.random.uniform(0.1,1,len(data)) 
        # print("shape(Z)", Z.shape)
        # Z.reshape(1,1,len(data))
        # print("shape(Z)", Z.shape)
        ...
        if coefs is None:
            ...
            D = (((self.codebook - data)/coefs) ** 2).sum(axis=-1)

        '''
        dist, x, y = 100, 0, 0
        for i in range(0, self.codebook.shape[0]):
            for j in range(0, self.codebook.shape[1]):
                somme = 0
                for k in range(0, len(data)) :
                    somme += (self.codebook[i][j][k] - data[k]) ** 2
                print("dist ({},{}) : {}".format(i,j,somme))
                if somme < dist:
                    dist, x, y = somme, i, j

        print("vainqueur ({},{}) : {}".format(x,y,dist))
        '''
        ...
        # Get index of nearest node (minimum distance)
        winner = np.unravel_index(np.argmin(D), D.shape)
        # print("winner {} : {}".format(winner, D[winner]))
        # quit()
        # Dynamic version
        ...
        # return winner, d[winner], dd[winner]
